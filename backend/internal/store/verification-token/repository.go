...
type IVTokenRepository interface {
	...
	GetOtpDataForUpdate(ctx context.Context, transaction *sql.Tx, verificationId uuid.UUID, verificationType auth.VerificationType) (*OTPVerificationData, error)
	// Get MagicLinkTokenPayload if magic link token found and is not expired
	GetValidMagicLinkDataForUpdate(ctx context.Context, transaction *sql.Tx, hashedToken []byte, verificationType auth.VerificationType) (*MagicLinkVerificationData, error)
