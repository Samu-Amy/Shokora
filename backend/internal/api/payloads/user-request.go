...
// TODO: finisci validazione (fai anche per stringhe generiche, tipo nomi prodotti e descrizioni)
...
type UserDataReq struct {
	...
	// ImageUrl  string    `json:"image_url,omitempty" validate:"omitempty,url,safe-chars"` // usare url (se l'url sarà conforme al controllo)?
}

type EmailFieldReq struct {
	Email string `json:"email" validate:"required,email,max=254,safe-chars"` // Length accepted by IETF
}

type PasswordFieldReq struct {
	Password string `json:"password" validate:"required,min=12,max=72,no-edge-spaces,valid-password,no-common-password"` // bcrypt clamp to 72 chars
}
...
type UpdatePasswordReq struct {
	OldPassword             string `json:"old_password" validate:"required,min=12,max=72,no-edge-spaces,valid-password"`
